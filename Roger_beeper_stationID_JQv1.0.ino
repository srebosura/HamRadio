//HAM Roger beeper and Station/Club ID JQ player v1.0 
//Samuel Rebosura
//Note: First time loading the sketch to a new Arduino board, needs to set default settings
...
//031-034 Station ID file
  ...
  #include <EEPROM.h>
    ...
    const int analog_busy_pin = A2;// connected to busy pin of JQplayer
    const int timer_select = 3;
    const int beep_select = 2;
    int rx_active = 1;
    int tone_signal = 0;
    int tone_ready = 0;
    int tx_hangtime = 25; //TX hangtime in milli secs
    ...
    int tone_enable = 1; //
    int club_ID_enable = 2; //club ID rotation enable by default
    int announcement_enable = 0;
    int set_passcode_enable = 0;
    int club_id_no = 31; // default is mp3 file 0031
...
void setup() {
  ...
  Serial.println("Roger Beeper DF_v1.0");
...
void check_busy(){//will just loop into checking the busy pin every 200msec and exit
  delay(100);
  while (analogRead(analog_busy_pin) >= 400){
    delay(200); 
  }
  delay (100);
}
...
void auto_ID_timer() {
    ...
    if (auto_id_timer <= 0) {
      ...
      if (club_ID_enable == 2) {
       ...
       tx_on(); 
       playfile(club_id_no); //play club ID rotation
       if (club_id_no == 34) {
        club_id_no = 30;
       }
       check_announcement();
...
void playfile(int file_num){ //function to play any file
  mute_off();
  mp3.playFileByIndexNumber(file_num); //Play file no. in memory
  check_busy();
}
...
void button_check(){
  ...
  if (digitalRead (timer_select) == 0 && tone_ready == 1){
   ...
   club_id_no = club_id_no+1;      
    ...
    if (club_id_no == 35){ club_ID_enable = 0;playfile(12);}
    if (club_id_no == 36){club_ID_enable = 2;playfile(11);club_id_no = 30;}
